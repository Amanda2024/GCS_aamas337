...
class Agents:
    def __init__(self, args):
        ...
        elif args.alg == 'vdn_graph':
            from policy.vdn_graph import VDN_GRAPH
            self.policy = VDN_GRAPH(args)
        elif args.alg == 'vdn_graph_2':
            from policy.vdn_graph_2 import VDN_GRAPH_2
            self.policy = VDN_GRAPH_2(args)
        else:
            raise Exception("No such algorithm")
        self.args = args


    def choose_action_dag_graph(self, obs, last_action, agent_num, father_action, avail_actions, epsilon, maven_z=None, evaluate=False):
        inputs = obs.copy()
    ...
    def train(self, batch, train_step, epsilon=None):  # coma needs epsilon for training
        ...
        loss_policy, loss_graph_actor, loss_graph_critic, loss_hA = self.policy.learn(batch, max_episode_len, train_step, epsilon)
        if train_step > 0 and train_step % self.args.save_cycle == 0:
            self.policy.save_model(train_step)
        return loss_policy, loss_graph_actor, loss_graph_critic, loss_hA   # log the loss after updating
