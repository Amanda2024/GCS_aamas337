...
class SSG_VDN():
    ...
    def train(self, model, follower_model, mixer, data):
        ...
        for mini_batch in data:
            ...
            target_q, _ = self.target_model(s_prime[0], h_out[0])
            target_q_ = target_q.reshape(self.arg_dict['rollout_len'], self.arg_dict['batch_size'], self.arg_dict['action_dim'])
            target_follower_q, _ = self.target_follower_model(s_prime[1], target_q_, h_out[1])
            ...
            target_q_[avail_u_next[0] == 0.0 ] = - 9999999
            target_follower_q_[avail_u_next[1] == 0.0 ] = - 9999999
            ...
            targets = r_total + self.arg_dict["gamma"] * s[0].squeeze(-1) * (q_total_target_max_q)  # (1, 60*32) tdone_mask is the same over each agent
    ...
    def _update_targets(self, model, follower_model, mixer):
    ...
    def save_models(self, path, model, follower_model, mixer):
