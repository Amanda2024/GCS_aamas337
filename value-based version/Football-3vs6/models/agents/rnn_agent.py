...
class RNNAgent(nn.Module):
    def __init__(self, input_shape, args, device=None):
        ...
        if device:
            self.device = device
        self.args = args
        self.hidden = args["lstm_size"]

        self.fc1 = nn.Linear(input_shape, self.hidden)
        self.rnn = nn.GRUCell(self.hidden, self.hidden)
        self.fc2 = nn.Linear(self.hidden, args["n_actions"])
        self.optimizer = optim.Adam(self.parameters(), lr=args["learning_rate"])

    def init_hidden(self):
        # make hidden states on same device as model
        return self.fc1.weight.new(1, self.hidden).zero_()
    ...
    def make_batch(self, data):
        ...
        state_lst_batch, state_prime_lst_batch, a_lst_batch, a1_lst_batch, avail_u_lst_batch, avail_u_next_lst_batch,\
        r_lst_batch,r_prime_lst_batch, done_lst_batch, father_batch, father_next_batch = [], [], [], [], [], [], [], [], [], [], []
        h_in_batch, h_out_batch = [], []
        for rollout in data:  # 3
            state_lst, state_prime_lst, a_lst, a1_lst, avail_u_lst, avail_u_next_lst,\
            r_lst,r_prime_lst, done_lst, father_lst = [], [], [], [], [], [], [], [], [], []
            h_in_lst, h_out_lst = [], []
            for transition in rollout:
                # s, a, m, r, s_prime, prob, done, need_move = transition
                obs_model, h_in, actions1, actions_onehot1, avail_u, reward_agency, reward_agency_prime, obs_prime, h_out, avail_u_next, done, father = transition

                state_lst.append(obs_model[:,0,:])
                state_prime_lst.append(obs_prime[:,0,:])
                a_lst.append(actions1)
                a1_lst.append(actions_onehot1)
                r_lst.append([reward_agency])
                r_prime_lst.append([reward_agency_prime])
                avail_u_lst.append(avail_u)
                avail_u_next_lst.append(avail_u_next)
                done_mask = [0]*len(obs_model) if done[0] else [1]*len(obs_model)
                done_lst.append([done_mask])
                father_lst.append(father)

                h_in_lst.append([i.tolist() for i in list(h_in)])
                h_out_lst.append([i.tolist() for i in list(h_out)])
                ...
                #     done_lst.append([done_mask])
                #     father_lst.append(father[i].tolist())
            ...
            father_next_lst = father_lst[1:] + father_lst[-1:]
            ...
            father_batch.append(father_lst)
            father_next_batch.append(father_next_lst)
        ...
        h_in = torch.tensor(h_in_batch, dtype=torch.float, device=self.device)  #2.5.3.1.128
        h_out = torch.tensor(h_out_batch, dtype=torch.float, device=self.device)  #2.5.3.1.128


        # obs_model:### batch_size\rollout_len\agents\x_dims 2.5.3.1.144
        #2.5.3.1.144, 2.5.3.1,   2.5.3.19,    2.5.3.19,   2.5.3.1,     2.5.3.1,             2.5.3.1.144,2.5.3.19,  2.5.3.1,   2.5.3.57
        obs_model, actions1, actions_onehot1, avail_u, reward_agency, reward_agency_prime,  obs_prime, avail_u_next, done_mask, father_ , father_next_= \
            torch.tensor(state_lst_batch, dtype=torch.float, device=self.device), \
            torch.tensor(a_lst_batch, dtype=torch.float, device=self.device),\
            torch.tensor(a1_lst_batch, dtype=torch.float, device=self.device),\
            torch.tensor(avail_u_lst_batch, dtype=torch.float, device=self.device),\
            torch.tensor(r_lst_batch, dtype=torch.float, device=self.device).permute(0, 1, 3, 2),\
            torch.tensor(r_prime_lst_batch, dtype=torch.float, device=self.device).permute(0, 1, 3, 2),\
            torch.tensor(state_prime_lst_batch, dtype=torch.float, device=self.device), \
            torch.tensor(avail_u_next_lst_batch, dtype=torch.float, device=self.device),\
            torch.tensor(done_lst_batch, dtype=torch.float, device=self.device).permute(0, 1, 3, 2),\
            torch.tensor(father_batch, dtype=torch.float, device=self.device),\
            torch.tensor(father_next_batch, dtype=torch.float, device=self.device)


        return obs_model, h_in, actions1, actions_onehot1, avail_u, reward_agency, reward_agency_prime, obs_prime, h_out, avail_u_next, done_mask, father_, father_next_
