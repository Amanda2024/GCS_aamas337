...
class TransEncoder(nn.Module):
    def __init__(self, n_xdims, nhead, num_layers):
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(n_xdims, nhead, 64)
...
class SingleLayerDecoder(nn.Module):
    def __init__(self, n_xdims, decoder_hidden_dim, node_num):
        ...
        self.fc_3 = nn.Linear(decoder_hidden_dim, 8)
    ...
    def forward(self, input):
        dot_l = self.fc_l(input) # bz, num, dim # 1.3.128
        ...
        final_sum = torch.tanh(tiled_l + tiled_r)  ## -1,1
        # final_sum = tiled_l + tiled_r
        # final_sum = torch.relu(tiled_l + tiled_r)
        x = torch.relu(self.fc_3(final_sum))  # 0.无穷
        logits = torch.sigmoid(self.fc_4(x).squeeze(-1)) # 0.1
...
class Actor_graph(nn.Module):
    ...
    def forward(self, src):
        ...
        log_softmax_logits_for_rewards = f.log_softmax(logits_for_rewards, dim=-1)  # 1.3.3
        log_softmax_logits_for_rewards_ = log_softmax_logits_for_rewards * graphs_gen
        log_softmax_logits_for_rewards_ = torch.sum(log_softmax_logits_for_rewards_, dim=(1,2))
        ...
        return encoder_output, samples, mask_scores, entropy, adj_prob, log_softmax_logits_for_rewards_, entropy_regularization
